...
def DeepPepPI(species):
    ...
    file_path = './PepPI dataset/'+species+'/cv positive peptide-protein pairs.txt'
    posi_pairs = load_data.load_pairs(file_path)

    file_path = './PepPI dataset/'+species+'/cv negative peptide-protein pairs.txt'
    ...
    fw = open('./Results/'+species+' DeepPePPI results ProteinBERT2.txt', 'a+')
    fw.write('tp\t'+str(ave_tp)+'\tfp\t'+str(ave_fp)+'\ttn\t'+str(ave_tn)+'\tfn\t'+str(ave_fn)+'\tAcc\t'+str(ave_acc)+'\tPrec\t'+str(ave_prec)+'\tRec\t'+str(ave_recall)+'\tMCC\t'+str(MCC)+'\tF1\t'+str(ave_f1_score)+'\tAUC\t'+str(ave_AUC)+'\tAUPR\t'+str(ave_AUPR)+'\n')


if __name__ == '__main__':
    species = 'Solanum lycopersicum' #Arabidopsis thaliana; Solanum lycopersicum
    DeepPepPI(species)